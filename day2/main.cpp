...
int main()
{
    ...
    spreadsheet.resize( 16 );

    std::size_t line_index = 0;

    while ( std::getline( ss, input_line ) )
    {
        auto & col_table = spreadsheet[ line_index++ ];
        col_table.reserve( 16 );

        std::stringstream line_ss( input_line );
        std::string token;
